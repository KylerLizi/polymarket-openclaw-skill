...
if (proxyUrl) {
  ...
  void import('global-agent/bootstrap');
}
...
} from './services/multi-account-executor.js';
export type {
  MultiAccountExecutorConfig,
  EligibleWalletInfo,
  MultiAccountTradeResult,
} from './services/multi-account-executor.js';
...
export {
  ...
  createRelayerRedeemServiceFromEnv,
} from './services/relayer-redeem-service.js';
export type {
  RelayerRedeemServiceConfig,
  RelayerRedeemResult,
  RelayerRedeemServiceFromEnvConfig,
} from './services/relayer-redeem-service.js';
...
// Wallet Utilities
export {
  deriveWalletsFromMnemonicRange,
...
export {
  ...
  createProvider,
} from './utils/rpc-provider.js';
export type { FailoverProviderConfig } from './utils/rpc-provider.js';
...
export class PolymarketSDK {
  ...
  /**
   * Get total account value (positions + USDC balance)
   *
   * 一次获取当前钱包的账户总价值，包括：
   * - 持仓市值 (从 Data API /value)
   * - USDC 余额 (从 CLOB API getBalanceAllowance)
   *
   * 注意：只能查询当前认证钱包的价值，因为 CLOB API 需要认证
   *
   * @returns 账户价值详情
   *
   * @example
   ...
   * console.log(`持仓: $${value.positionsValue}, 余额: $${value.usdcBalance}, 总计: $${value.totalValue}`);
   * ```
   */
  async getAccountTotalValue(): Promise<{
    positionsValue: number;
    usdcBalance: number;
    totalValue: number;
  }> {
    const walletAddress = this.tradingService.getAddress();

    const [positionValue, balanceInfo] = await Promise.all([
      this.dataApi.getAccountValue(walletAddress),
      this.tradingService.getBalanceAllowance('COLLATERAL'),
    ]);

    const usdcBalance = parseFloat(balanceInfo.balance) || 0;

    return {
      positionsValue: positionValue.value,
      usdcBalance,
      totalValue: positionValue.value + usdcBalance,
    };
  }
