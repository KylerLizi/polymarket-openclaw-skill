...
export class RealtimeServiceV2 extends EventEmitter {
  ...
  connect(): this {
    ...
    // Workaround: library bug — `this.autoReconnect = args.autoReconnect || true`
    // means `false || true = true`, so autoReconnect is always true regardless of config.
    // Override directly on the instance to prevent infinite rapid reconnect loops on 429.
    (this.client as any).autoReconnect = false;
