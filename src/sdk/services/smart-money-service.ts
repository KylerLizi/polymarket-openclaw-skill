...
export interface AutoCopyTradingOptions {
  ...
  /** Allow duplicate BUY orders even if already holding the same conditionId+outcome (default: false) */
...
export class SmartMoneyService {
  ...
  private _myPositionsCache: Position[] | null = null;
  private _myPositionsCacheAt: number = 0;
  ...
  private async getMyPositionsCached(ttlMs: number = 10_000): Promise<Position[]> {
    const now = Date.now();
    if (this._myPositionsCache && now - this._myPositionsCacheAt < ttlMs) {
      return this._myPositionsCache;
    }

    // In Safe(proxy) mode, balances/positions are attributed to funder (Safe address), not the EOA signer.
    const funder = (this.tradingService as any).getFunderAddress?.() as string | null | undefined;
    const myAddress = (funder || this.tradingService.getAddress()).toLowerCase();

    const positions = await this.walletService.getWalletPositions(myAddress).catch(() => []);
    this._myPositionsCache = positions;
    this._myPositionsCacheAt = now;
    return positions;
  }

  private async isAlreadyHolding(conditionId?: string, outcome?: string): Promise<boolean> {
    if (!conditionId || !outcome) return false;
    const positions = await this.getMyPositionsCached();
    const c = conditionId.toLowerCase();
    const o = outcome.toLowerCase();
    return positions.some(p =>
      (p.conditionId || '').toLowerCase() === c &&
      (p.outcome || '').toLowerCase() === o &&
      (p.size ?? 0) > 0
    );
  }

  private clampPrice(price: number): number {
    // Polymarket price must be strictly between 0 and 1, and server enforces ~[0.001, 0.999]
    const MIN = 0.001;
    const MAX = 0.999;
    const safe = Math.min(MAX, Math.max(MIN, price));

    // Avoid floating point artifacts like 1.0000000002 causing rejection
  ...
  async startAutoCopyTrading(options: AutoCopyTradingOptions): Promise<AutoCopyTradingSubscription> {
    ...
    if (options.topN && options.topN > 0) {
      ...
      const merged = targetAddresses.concat(topAddresses);
      const seen: Record<string, true> = {};
      targetAddresses = merged.filter((a) => {
    ...
    const allowDuplicateBuys = options.allowDuplicateBuys ?? false;

    // When duplicate BUYs are allowed, enforce a per-market cap to avoid runaway repeats.
    // Keyed by conditionId (market). Counts successful BUY orders we submitted.
    const buyCountByConditionId = new Map<string, number>();
    const MAX_BUYS_PER_MARKET_WHEN_DUPLICATE_ALLOWED = 10;
    ...
    const subscription = this.subscribeSmartMoneyTrades(
      async (trade: SmartMoneyTrade) => {
        ...
        try {
          ...
          // Cap: when duplicate buys are allowed, limit max BUY orders per market (conditionId)
          if (allowDuplicateBuys && trade.side === 'BUY') {
            const key = String(trade.conditionId || '').toLowerCase();
            if (key) {
              const current = buyCountByConditionId.get(key) ?? 0;
              if (current >= MAX_BUYS_PER_MARKET_WHEN_DUPLICATE_ALLOWED) {
                stats.tradesSkipped++;
                const skipResult: OrderResult = {
                  success: false,
                  errorMsg: `Skipped: max BUY orders per market reached (${MAX_BUYS_PER_MARKET_WHEN_DUPLICATE_ALLOWED})`,
                };
                options.onTrade?.(trade, skipResult);
                return;
              }
            }
          }

          // Avoid duplicate positions: if we already hold the same conditionId+outcome, skip BUY.
          if (trade.side === 'BUY' && !allowDuplicateBuys) {
            const alreadyHolding = await this.isAlreadyHolding(trade.conditionId, trade.outcome);
            if (alreadyHolding) {
              stats.tradesSkipped++;
              const skipResult: OrderResult = {
                success: false,
                errorMsg: 'Skipped: already holding this position',
              };
              options.onTrade?.(trade, skipResult);
              return;
            }
          ...
          const slippagePriceRaw = trade.side === 'BUY'
            ...
            : trade.price * (1 - maxSlippage);
          const slippagePrice = this.clampPrice(slippagePriceRaw);

          const usdcAmount = copyValue; // Already calculated above

          // For SELL: only sell if we currently hold this position, and never exceed our holdings.
          // Polymarket enforces minimum SELL size of 5 shares.
          let sellShares: number | null = null;
          if (trade.side === 'SELL') {
            const positions = await this.getMyPositionsCached();
            const c = (trade.conditionId || '').toLowerCase();
            const o = (trade.outcome || '').toLowerCase();
            const t = (trade.tokenId || '').toLowerCase();

            const held = positions.find(p => {
              const pc = (p.conditionId || '').toLowerCase();
              const po = (p.outcome || '').toLowerCase();
              const pa = (p.asset || '').toLowerCase();
              const matchByConditionOutcome = c.length > 0 && o.length > 0 && pc === c && po === o;
              const matchByTokenId = t.length > 0 && pa === t;
              return (matchByConditionOutcome || matchByTokenId) && (p.size ?? 0) > 0;
            });
            ...
            if (!Number.isFinite(heldShares) || heldShares <= 0) {
              ...
              const skipResult: OrderResult = {
                success: false,
                errorMsg: 'Skipped: no position to sell',
              };
              options.onTrade?.(trade, skipResult);
              return;
            }

            const desiredShares = Number(copySize);
            sellShares = Math.min(heldShares, Number.isFinite(desiredShares) ? desiredShares : 0);
            ...
            if (!Number.isFinite(sellShares) || sellShares <= 0 || sellValue < MIN_SELL_VALUE_USDC) {
              ...
              const skipResult: OrderResult = {
                success: false,
                errorMsg: `Skipped: sell value too small ($${sellValue.toFixed(2)} < $${MIN_SELL_VALUE_USDC} minimum)`,
              };
              options.onTrade?.(trade, skipResult);
              return;
            }
          ...
          if (dryRun) {
            ...
            result = await this.tradingService.createMarketOrder({
              tokenId,
              side: trade.side,
              amount: trade.side === 'SELL' ? (sellShares as number) : usdcAmount,
              price: slippagePrice,
              orderType,
            });
          }
          ...
          if (result.success) {
            ...
            if (trade.side === 'BUY') {
              ...
              if (allowDuplicateBuys) {
                const key = String(trade.conditionId || '').toLowerCase();
                if (key) {
                  const current = buyCountByConditionId.get(key) ?? 0;
                  buyCountByConditionId.set(key, current + 1);
                }
              }
            }

            // Bust positions cache so next trade sees the updated state
            this._myPositionsCacheAt = 0;
          } else {
            stats.tradesFailed++;
          }
