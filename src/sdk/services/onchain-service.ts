...
import { FailoverProvider, DEFAULT_POLYGON_RPC_URLS } from '../utils/rpc-provider';
...
export interface OnchainServiceConfig {
  ...
  privateKey: string;
  /** RPC URL (default: Polygon mainnet) - single URL for backward compatibility */
  rpcUrl?: string;
  /** List of RPC URLs for failover (takes precedence over rpcUrl if provided) */
  rpcUrls?: string[];
  /** Enable RPC failover (default: true) */
...
export class OnchainService {
  ...
  constructor(config: OnchainServiceConfig) {
    // Use failover provider by default
    const enableFailover = config.enableFailover !== false;
    
    if (enableFailover) {
      // Build RPC URL list: custom rpcUrls > single rpcUrl > defaults
      const rpcUrls = config.rpcUrls?.length 
        ? config.rpcUrls 
        : config.rpcUrl 
          ? [config.rpcUrl, ...DEFAULT_POLYGON_RPC_URLS.filter(u => u !== config.rpcUrl)]
          : DEFAULT_POLYGON_RPC_URLS;
      this.provider = new FailoverProvider({ rpcUrls }) as any;
    } else {
      const rpcUrl = config.rpcUrl || DEFAULT_POLYGON_RPC_URLS[0];
      this.provider = new ethers.providers.JsonRpcProvider(rpcUrl);
    }
    
    this.wallet = new ethers.Wallet(config.privateKey, this.provider);

    // Initialize CTFClient with config (CTFClient has its own failover)
